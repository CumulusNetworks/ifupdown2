...
class bridge(moduleBase):
    ...
    def _get_bridge_port_condone_regex(self, ifaceobj, get_string=False):
        bridge_port_condone_regex = ifaceobj.get_attr_value_first(
            'bridge-ports-condone-regex')
    ...
    def _enable_disable_ipv6(self, port, enable='1'):
        try:
            self.write_file(
                '/proc/sys/net/ipv6/conf/%s/disable_ipv6' % port, enable)
        except Exception, e:
                ...
                for p in portlist:
                    try:
                        ...
                        if attrname == 'bridge-portmcrouter':
                            portattrs[port].update(
                                {dstattrname: utils.boolean_support_binary(val)})
                        else:
    ...
    def _apply_bridge_vlan_aware_port_settings_all(self, bportifaceobj,
        ...
        if bport_access:
            ...
            allow_untagged = bportifaceobj.get_attr_value_first(
                'bridge-allow-untagged') or 'yes'
    ...
    def _apply_bridge_port_settings_all(self, ifaceobj,
            ...
            for bportifaceobj in bportifaceobjlist:
                ...
                try:
                    ...
                    if bridge_vlan_aware:
                        ...
                        self._check_untagged_bridge(
                            ifaceobj.name, bportifaceobj, ifaceobj_getfunc)
                except Exception, e:
                    err = True
        ...
        if err:
            raise Exception('%s: errors applying port settings' %
                            ifaceobj.name)
    ...
    def _check_untagged_bridge(self, bridgename, bridgeportifaceobj, ifaceobj_getfunc):
        if bridgeportifaceobj.link_kind & ifaceLinkKind.VLAN:
            lower_ifaceobj_list = ifaceobj_getfunc(
                bridgeportifaceobj.lowerifaces[0])
            if lower_ifaceobj_list and lower_ifaceobj_list[0] and \
                ...
                self.logger.warn(
                    '%s: untagged bridge not found. Please configure a bridge with untagged bridge ports to avoid Spanning Tree Interoperability issue.' % bridgename)
            ...
            if ifaceobj.get_attr_value_first('bridge-vlan-aware') == 'yes':
                ...
                if (bridge_just_created or
                    ...
                    if not bridge_just_created:
                        ifaceobj.module_flags[self.name] = ifaceobj.module_flags.setdefault(
                            self.name, 0) | bridgeFlags.PORT_PROCESSED_OVERRIDE
    ...
    def _query_running_vidinfo_compat(self, ifaceobjrunning, ports):
        ...
        if running_bridge_vids:
            running_attrs['bridge-vids'] = ','.join(
                self._compress_into_ranges(running_bridge_vids))
    ...
    def _query_running_attrs(self, ifaceobjrunning, ifaceobj_getfunc,
        ...
        tmpbridgeattrdict = self.brctlcmd.get_bridge_attrs(
    ...
    def _query_check_bridge(self, ifaceobj, ifaceobjcurr,
        ...
        filterattrs = ['bridge-vids', 'bridge-port-vids',
                       'bridge-port-pvids']
        bridge_port_condone_regex = self._get_bridge_port_condone_regex(
            ifaceobj)
        for k in Set(ifaceattrs).difference(filterattrs):
            ...
            if k == 'bridge-vlan-aware':
                ...
                bridge_port_condone_regex_str = self._get_bridge_port_condone_regex(
                    ifaceobj, True)
                ifaceobjcurr.update_config_with_status(
                    k, bridge_port_condone_regex_str, 0)
            elif k == 'bridge-ports':
                ...
                if running_port_list and bridge_port_list:
                    ...
                    if bridge_port_condone_regex:
                        ...
                        difference = [
                            port for port in difference if not bridge_port_condone_regex.match(port)]
                        # Tag all condoned ports in brackets in output
                        running_port_list_pretty = ["(%s)" % port if port not in bridge_port_list and bridge_port_condone_regex.match(
                            port) else port for port in running_port_list]
                    if not difference:
                        portliststatus = 0
                    ifaceobjcurr.update_config_with_status('bridge-ports',
                                                           ' '.join(
                                                               running_port_list_pretty)
    ...
    def _query_running_bridge_port(self, ifaceobjrunning,
        ...
        if bridgeifaceobjlist:
            ...
            bridge_pvid = bridgeifaceobjlist[0].get_attr_value_first(
                'bridge-pvid')
        ...
        for attrl in [['mcqifaddr', 'bridge-mcqifaddr'],
            ...
            if value and not utils.is_binary_bool(value):
                if attrl[0] in runningattrs:
                    bool = utils.get_boolean_from_string(
                        runningattrs[attrl[0]])
