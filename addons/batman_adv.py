...
class batman_adv (moduleBase):
    ...
    def _get_batman_ifaces_ignore_regex(self, ifaceobj):
        ifaces_ignore_regex = ifaceobj.get_attr_value_first(
            'batman-ifaces-ignore-regex')
    ...
    def _get_batman_attr(self, ifaceobj, attr):
        if attr not in self._batman_attrs:
            raise ValueError(
                "_get_batman_attr: Invalid or unsupported B.A.T.M.A.N. adv. attribute: %s" % attr)
    ...
    def _read_current_batman_attr(self, ifaceobj, attr):
        if attr not in self._batman_attrs:
            raise ValueError(
                "_read_current_batman_attr: Invalid or unsupported B.A.T.M.A.N. adv. attribute: %s" % attr)

        attr_file_name = self._batman_attrs[attr]['filename']
        attr_file_path = "/sys/class/net/%s/mesh/%s" % (
            ifaceobj.name, attr_file_name)
        try:
            ...
            raise Exception(
                "_read_current_batman_attr: Integer value expected, got: %s" % value)

    def _set_batman_attr(self, ifaceobj, attr, value):
        if attr not in self._batman_attrs:
            raise ValueError(
                "_set_batman_attr: Invalid or unsupported B.A.T.M.A.N. adv. attribute: %s" % attr)
    ...
    def _batctl_if(self, bat_iface, mesh_iface, op):
        if op not in ['add', 'del']:
            raise Exception(
                "_batctl_if() called with invalid \"op\" value: %s" % op)

        try:
            self.logger.debug("Running batctl -m %s if %s %s" %
                              (bat_iface, op, mesh_iface))
            batctl_output = subprocess.check_output(
                ["batctl", "-m", bat_iface, "if", op, mesh_iface], stderr=subprocess.STDOUT)
        except subprocess.CalledProcessError as c:
            raise Exception("Command \"batctl -m %s if %s %s\" failed: %s" %
                            (bat_iface, op, mesh_iface, c.output))
    ...
    def _find_member_ifaces(self, ifaceobj, ignore=True):
        ...
        batctl_fh = subprocess.Popen(
            ["batctl", "-m", ifaceobj.name, "if"], bufsize=4194304, stdout=subprocess.PIPE).stdout
    ...
    def _up(self, ifaceobj):
        ...
        if len(batman_ifaces) == 0:
            raise Exception(
                "None of the configured batman interfaces are available!")
    ...
    def _query_check(self, ifaceobj, ifaceobjcurr):
        ...
        iface_ignore_re_str = ifaceobj.get_attr_value_first(
            'batman-ifaces-ignore-regex')
        iface_ignore_re = self._get_batman_ifaces_ignore_regex(ifaceobj)
        ...
        ifaceobjcurr.update_config_with_status(
            'batman-ifaces', ifaces_str, ifaces_ok)
        ifaceobjcurr.update_config_with_status(
            'batman-ifaces-ignore-regex', iface_ignore_re_str, 0)
        ...
        for attr in self._batman_attrs:
            ...
            ifaceobjcurr.update_config_with_status(
                'batman-%s' % attr, value_curr, value_ok)
