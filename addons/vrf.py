...
class vrf(moduleBase):
    ...
    def __init__(self, *args, **kargs):
        ...
        self.vrf_close_socks_on_down = policymanager.policymanager_api.get_module_globals(
            module_name=self.__class__.__name__, attr='vrf-close-socks-on-down')
            ...
            with open(self.iproute2_vrf_filename, 'r+') as vrf_map_fd:
                ...
                for l in lines:
                    ...
                    try:
                        ...
                        self.logger.info(
                            'vrf: iproute2_vrf_map: unable to parse %s (%s)' % (l, str(e)))
                        pass
    ...
    def _iproute2_vrf_map_open(self, sync_vrfs=False, append=False):
        ...
        try:
            self.iproute2_vrf_map_fd = open(self.iproute2_vrf_filename,
                                            '%s' % fmode)
            fcntl.fcntl(self.iproute2_vrf_map_fd,
    ...
    def _create_vrf_dev(self, ifaceobj, vrf_table):
        if not self.ipcmd.link_exists(ifaceobj.name):
            ...
            if not vrf_table.isdigit():
                self.log_error(
                    'vrf-table must be an integer or \'auto\'', ifaceobj)
    ...
    def _kill_ssh_connections(self, ifacename):
        try:
            ...
            pstree = list(reversed(utils.exec_command(
                '/usr/bin/pstree -Aps %s' % os.getpid()).strip().split('---')))
            ...
            if pid in proc:
                try:
                    ...
                    self.logger.info(
                        "fork error : %s [%d]" % (e.strerror, e.errno))
                if (forkret == 0):  # The first child.
                    try:
                        os.setsid()
                        self.logger.info(
                            "%s: ifreload continuing in the background" % ifacename)
                    except OSError, (err_no, err_message):
                        self.logger.info(
                            "os.setsid failed: errno=%d: %s" % (err_no, err_message))
